...
from dataset import PathMNIST, OCTMNIST, PneumoniaMNIST, ChestMNIST
...
def npy_loader(path):
    sample = np.load(path) #im.fromarray(np.load(path)) #transforms.ToPILImage(torch.from_numpy(np.load(path)))
...
def return_data(args):
    ...
    if name.lower() == 'celeba':
        if train:
            ...
            root = os.path.join(dset_dir, 'CelebASmall')
        else:
    ...
    elif name.lower() == 'medmnist':
        if train:
            print('****loading Train data now****')
            root = os.path.join('data', 'Medmnist')
        else:
            print('****loading Testing data now****')
            root = os.path.join(dset_dir, 'Test')
        transform = transforms.Compose([
        ...
        if args.train:
            train_data = PathMNIST(root, split='train',transform=transform, download=True)
        else:
            train_data = PathMNIST(root, split='test',transform=transform, download=True)
